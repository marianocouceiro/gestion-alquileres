...
const GestShared = (function () {
  ...
  const NAV = [
    ...
    { key:'visitas',        href:'visitas.html',         label:'Visitas' },
    { key:'ventas',         href:'ventas.html',          label:'Propiedades' },
    { key:'tasaciones',     href:'tasaciones.html',      label:'Tasaciones' },
    { key:'contratos',      href:'index.html',           label:'Contratos' },
    { key:'administracion', href:'administracion.html',  label:'Administración' },
    { key:'banco',          href:'banco.html',           label:'Banco' },
    { key:'tareas',         href:'tareas.html',          label:'Tareas' },
    { key:'configuracion',  href:'configuracion.html',   label:'Configuración' },
    { key:'ayuda',          href:'ayuda.html',           label:'Ayuda' },
  ];
...
.nav-lnk{
  display:inline-block!important;outline:0!important;cursor:pointer!important;border:none!important;
  padding:0 13px!important;height:32px!important;line-height:32px!important;
  border-radius:7px!important;font-size:.76rem!important;font-weight:500!important;
  color:#fff!important;background:linear-gradient(135deg,#e65100,#ff9800)!important;
  box-shadow:0 2px 8px rgba(230,81,0,0.35)!important;
  text-decoration:none!important;white-space:nowrap!important;transition:box-shadow .2s,transform .1s!important;
}
.nav-lnk.active{background:linear-gradient(135deg,#ff6d00,#ffb300)!important;color:#000!important;box-shadow:0 4px 14px rgba(230,81,0,0.65)!important;transform:translateY(-1px)!important}
...
`;
  ...
  function initHeader(activePage) {
    ...
    const navHtml = NAV.map(n => {
      ...
      return `<a href="${n.href}" class="gs-nav-btn${active}">${n.label}</a>`;
    }).join('');
